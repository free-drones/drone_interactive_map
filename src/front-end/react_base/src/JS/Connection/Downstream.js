...
export function setMode(mode, view = null, callback = null) {
    if (mode === MODE.automatic && !isValidView(view))
        throw new Error('Invalid view. When requesting automatic mode a valid view must be given.');
...
export function translateView(view) {
    if (!isValidView(view))
        throw new Error('Invalid view. View must contain properties "upLeft", "upRight", "downReft", "downRight" and "center", each with a "lat" and "lng" property.');
...
export function translateCoordinate(coordinate) {
    if (!isValidCoordinate(coordinate))
        throw new Error('Invalid coordinate. Coordinate must contain properties "lat" and "lng".');
...
export function isValidCoordinate(coordinate) {
    return coordinate.lat !== undefined &&
    ...
    return (reply) => {
        switch(reply.fcn) {
            case "ack":
                ...
                break;
            case "error":
                Storage.store.dispatch(Storage.messagesActions.addMessage("error", "Error " + reply.fcn_name, reply.error_report));
                break;
            default:
                throw new Error("Received unexpected function '" + reply.fcn + "' in reply.");
        }
    }
}

export default {TYPE, MODE, connect, checkAlive, disconnect, setArea, requestView, requestPriorityView, clearImageQueue, setMode, getInfo, getQueueETA, callbackWrapper};
