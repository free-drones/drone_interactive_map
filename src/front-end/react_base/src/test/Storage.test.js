...
test('sets map position', () => {
    ...
    Storage.store.dispatch(setMapPosition({
        upLeft: {
            lat: 58.409049,
            lng: 15.609948
        },
...
test('adds an active picture', () => {
    Storage.store.dispatch(addActivePicture({
        type: "RGB",
        prioritized: true,
        imageID: 1,
        url: "URL",
...
test('set bounds', () => {
    Storage.store.dispatch(setMapBounds([[1,2],[2,1]]))
    expect(Storage.store.getState().mapBounds).toEqual([[1,2],[2,1]]);
});
...
test('set bad bounds', () => {
    try{
        ...
        expect(e.message).toBe("Bounds should be a pair of coordinates. Ex: [[1,2], [2,1]]")
    }
});
